...
class CKAN(nn.Module):
    ...
    def get_user_full_emb(self, users, user_triple_set):
        ...
        if self.args.use_raw_id_emb:
            user_id_emb = self.user_emb(users)
            user_cross_emb = user_id_emb
    ...
    def get_item_full_emb(self, items, item_triple_set):
        ...
        if self.args.use_raw_id_emb:
            item_id_emb = self.item_emb(items)
            item_cross_emb = item_id_emb
    ...
    def forward(
        ...
        if self.args.use_user_kg_loss:
            ...
            user_cos_loss = 0.05 * user_cos_loss


        """ item 知识图谱表征模块 """
        item_cos_loss = 0
        if self.args.use_item_kg_loss:
            h_emb_list = []
            r_emb_list = []
            t_emb_list = []
