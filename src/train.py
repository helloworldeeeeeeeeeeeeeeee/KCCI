...
def train(args, data_info):
    """
        data_info: [train_data, eval_data, test_data, n_entity, n_relation, user_triple_sets]
            
            train_data: 每行为 user_id, item_id, labels 
            user_triple_sets: 用户的历史行为构成三元组, dict 类型, user_id: [(h, r, t), (h, r, t)]
    ...
    for step in range(args.n_epoch):
        ...
        while start < train_data.shape[0]:
            ...
            scores, contrastive_loss, user_cos_loss, item_cos_loss = model(*_get_feed_data(args, train_data, user_triple_set, item_triple_set, start, start + args.batch_size))
            
            # 计算 train auc 和 loss

            loss = loss_func(scores, labels)

            loss = loss + 0.1 * contrastive_loss + 0.1 * user_cos_loss + 0.1 * item_cos_loss
...
def ctr_eval(args, model, data, user_triple_set, item_triple_set):
    """
        输入 eval 数据或 test 数据, 对模型进行评估, 得到 auc f1 loss
    """
    ...
    while start < data.shape[0]:
        ...
        scores, contrastive_loss, user_cos_loss, item_cos_loss = model(*_get_feed_data(args, data, user_triple_set, item_triple_set, start, start + args.eval_batch_size))
...
def topk_eval(args, model, train_data, test_data, user_triple_set, item_triple_set):
    ...
    for user in user_list:
        ...
        while start + args.eval_batch_size <= len(test_item_list):
            ...
            scores, contrastive_loss, user_cos_loss, item_cos_loss = model(*_get_feed_data(args, input_data, user_triple_set, item_triple_set, 0, args.eval_batch_size))
...
def _init_model(args, data_info):
    ...
    model = CKAN(args, n_entity, n_relation, n_user, n_item)
    if args.use_cuda:
        print("use cuda!")
        model.cuda()
...
def _get_triple_tensor(args, objs, triple_set):
    ...
    """
        输入:
            objs: 实际上就是 train_data 中第 0 列, 即 user_id
            triple_set: user_triple_set, dict 类型, key 是 user_id, value 是 List 类型, 每个元素是 (h,r,t) 三元组
        输出:
            [h,r,t], 其中 h r t 分别都是 List 类型, 每个元素都是 LongTensor.
            以 h 为例, 维度为 [layers, batch_size, len(objs)]
    """
